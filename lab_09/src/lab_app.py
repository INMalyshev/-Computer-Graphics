...
class LabApp(App):
    def __init__(self):
        ...
        # Кнопки
        self.process_button = MyButton(self, 'Задать фигуру руками', self._start_make_figure_process)
        ...
        self.change_center_button = MyButton(self, 'Добавить точку фигуры', self._handle_add_figure_dot_button)
        self.change_center_button.place(relx=0.8, rely=0.8, relheight=0.05, relwidth=0.2)

        self.finish_figure_button = MyButton(self, 'Замкнуть фигуру', self._handle_finish_figure_button)
        self.finish_figure_button.place(relx=0.8, rely=0.85, relheight=0.05, relwidth=0.2)
    ...
    def _handle_add_figure_dot_button(self, event=None):
        if self.hand_mod:
            return
    ...
    def _handle_finish_figure_button(self, event=None):
        ...
        if len(self.position.data[-1].dots) > 2:
            ...
            showerror('Ошибка', 'Фигура должна иметь не менее 3 вершин.')
    ...
    def _start_make_figure_process(self, event=None):
        ...
        binds.append(self.canvas.bind("<Shift-Button-1>", lambda event: add_perpendicular_dot(self, event)))
        binds.append(
            self.canvas.bind("<Shift-Motion>", lambda event: create_perpendicular_prompt_line(self, event)))